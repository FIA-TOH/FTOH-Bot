...
export const bestTimes: { [key: string]: [number, string, string] } = {
  ...
  hockenheimring: [999.233, 'Jean Dany-Vegne', trackNameMapping['hockenheimring']],
  ...
  barcelona: [37.7, 'Ximbastian Vettel', 'Circuit de Barcelona-Catalunya by Rodri'],
  daytona: [81.234, 'Rodri&Samusca', '24H Daytona edited by Rodri&Samusca'],
  cano: [60.0, 'Ximbastian Vettel', 'Circuito Urbano de La Villa Cano - By Ximb'],
};
...
export const getBestTime = (trackName: string): [number, string, string] | null => {
...
export const updateBestTime = (trackName: string, newTime: number, driverName: string) => {
  ...
  if (trackNameMapping.hasOwnProperty(abbreviatedTrackName)) {
    ...
    log(`The track ${abbreviatedTrackName} wasn't found on the mapping to update the best time.`);
  }
};
...
export const clearBestTime = (trackName: string, newTime: number, driverName: string) => {
  ...
  if (trackNameMapping.hasOwnProperty(abbreviatedTrackName)) {
    ...
    log(`The track ${abbreviatedTrackName} wasn't found on the mapping to clear the best time.`);
  }
};
